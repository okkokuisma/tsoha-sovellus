...
def get_course_results(user_id, course_id):
    sql = "SELECT e.id, e.name, r.result, r.max_points FROM results r, exercises e WHERE r.exercise_id IN (SELECT id FROM exercises WHERE course_id=:course_id AND visible=1) AND user_id=:user_id AND r.exercise_id=e.id"
